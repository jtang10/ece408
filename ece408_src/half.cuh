...
__global__ void forward_shared_unroll_one(float *y, const float *x, const float *k,
  ...
  #define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
	...
	__shared__ half shmem_X[TILE_WIDTH_ONE * TILE_WIDTH_ONE];
	__shared__ half shmem_K[TILE_WIDTH_ONE * TILE_WIDTH_ONE];
  ...
  for (int i = 0; i < (numMatACol - 1) / TILE_WIDTH_ONE + 1; ++i) { // ceil
      ...
      shmem_K[location] = __float2half_rn(k[threadIdx.y * numMatACol + temp_col]);
  	} else {
  		shmem_K[location] = __float2half_rn(0.0f);
  	}
    ...
    int X_q = (temp_row % (K * K)) % K;
  	...
  	if (temp_row < numMatACol && col < H_out * W_out) {
  		shmem_X[location] = __float2half_rn(x4d(blockIdx.z, X_c, X_h + X_p, X_w + X_q));
  	} else {
  		shmem_X[location] = __float2half_rn(0.0f);
  	}
    ...
    #pragma unroll
      ...
      y[blockIdx.z * (M * H_out * W_out) + threadIdx.y * (H_out * W_out) + col] = __half2float(acc);
  	}
  }
