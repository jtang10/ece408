...
__global__ void forward_shared_unroll_one(float *y, const float *x, const float *k,
  ...
  #define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
	...
	__shared__ float shmem_X[TILE_WIDTH_ONE];
	__shared__ float shmem_K[TILE_WIDTH_ONE];
  ...
  // #pragma unroll
  for (int i = 0; i < numMatACol; ++i) { // ceil

  	// int temp_col = i * TILE_WIDTH_ONE + threadIdx.x;
  	// int temp_row = i * TILE_WIDTH_ONE + threadIdx.y;

    if (threadIdx.y == 0) {
    	if (blockIdx.y * blockDim.y + threadIdx.x < M) { //  && row < M
        shmem_K[threadIdx.x] = k[(blockIdx.y * blockDim.y + threadIdx.x) * numMatACol + i];
    	} else {
    		shmem_K[threadIdx.x] = 0;
    	}

    	// int X_b = b;
      // x[(i3) * (C * H * W) + (i2) * (H * W) + (i1) * (W) + i0]b
      ...
      int X_q = (i % (K * K)) % K;
    	...
    	if (col < H_out * W_out) {
    		shmem_X[threadIdx.x] = x4d(blockIdx.z, 0, X_h + X_p, X_w + X_q);
    	} else {
    		shmem_X[threadIdx.x] = 0;
    	}
    }

  	__syncthreads();

		acc += shmem_K[threadIdx.y] * shmem_X[threadIdx.x];
	   __syncthreads();
   }
