...
__constant__ float K_const[14112]; // max of MCKK == 24 * 12 * 7 * 7
...
__global__ void forward_shared_unroll_one(float *y, const float *x, const float *k,
  ...
  for (int i = 0; i < (numMatACol - 1) / TILE_WIDTH_ONE + 1; ++i) { // ceil
    ...
    #pragma unroll
    for (int j = 0; j < TILE_WIDTH_ONE; j++) {

    	int temp_row = i * TILE_WIDTH_ONE + j;
      ...
      int X_q = (temp_row % (K * K)) % K;
    	...
    	if (temp_row < numMatACol && col < H_out * W_out) {
    		shmem_X[j * TILE_WIDTH_ONE + threadIdx.x] = x4d(blockIdx.z, X_c, X_h + X_p, X_w + X_q);
    	} else {
    		shmem_X[j * TILE_WIDTH_ONE + threadIdx.x] = 0;
    	}
    ...
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      ...
      acc0 += K_const[0 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }

    #pragma unroll
  	for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
  		// acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc1 += K_const[1 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
  	}

    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc2 += K_const[2 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }

    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc3 += K_const[3 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }

    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc4 += K_const[4 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }

    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc5 += K_const[5 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }

    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc6 += K_const[6 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }

    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc7 += K_const[7 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }
    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc8 += K_const[8 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }

    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc9 += K_const[9 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }

    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc10 += K_const[10 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }

    #pragma unroll
    for (int q = 0; q < TILE_WIDTH_ONE; ++q) {
      // acc += shmem_K[threadIdx.y * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
      acc11 += K_const[11 * numMatACol + i * TILE_WIDTH_ONE + q] * shmem_X[q * TILE_WIDTH_ONE + threadIdx.x];
    }
      ...
      y[blockIdx.z * (M * H_out * W_out) + 0 * (H_out * W_out) + col] = acc0;
  	}
    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 1 * (H_out * W_out) + col] = acc1;
  	}
    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 2 * (H_out * W_out) + col] = acc2;
  	}
    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 3 * (H_out * W_out) + col] = acc3;
  	}
    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 4 * (H_out * W_out) + col] = acc4;
  	}

    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 5 * (H_out * W_out) + col] = acc5;
  	}
    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 6 * (H_out * W_out) + col] = acc6;
  	}
    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 7 * (H_out * W_out) + col] = acc7;
  	}
    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 8 * (H_out * W_out) + col] = acc8;
  	}
    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 9 * (H_out * W_out) + col] = acc9;
  	}

    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 10 * (H_out * W_out) + col] = acc10;
  	}
    if (row < M && col < W_out * H_out) {
      y[blockIdx.z * (M * H_out * W_out) + 11 * (H_out * W_out) + col] = acc11;
  	}
  }
...
__global__ void forward_shared_unroll_two(float *y, const float *x, const float *k,
  ...
  for (int i = 0; i < (numMatACol - 1) / TILE_WIDTH_TWO + 1; ++i) { // ceil
    ...
    #pragma unroll
      ...
      acc += K_const[row * numMatACol + i * TILE_WIDTH_TWO + q] * shmem_X[q * TILE_WIDTH_TWO + threadIdx.x];
    }
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y,
  ...
  cudaMemcpyToSymbol(K_const, w.dptr_, M*C*K*K*sizeof(float), 0, cudaMemcpyHostToDevice);

  if (M == 24) {
    dim3 gridDim(((H_out*W_out-1)/TILE_WIDTH_TWO+1), ((M-1)/TILE_WIDTH_TWO+1), B);
    dim3 blockDim(TILE_WIDTH_TWO, 1, 1);
